                        ...
                        <div className="bg-[#111827] p-6 shadow rounded-xl border border-[#1f2937] flex flex-col">
                            ...
                            <div className="bg-[#0b0e14] border border-[#1f2937] p-4 rounded-lg mb-6 flex-1">
                                <div className="flex items-center mb-2">
                                    ...
                                    {waStatus.connected ? (
                                        <span className="flex items-center text-green-400 text-sm font-medium"><CheckCircle className="h-4 w-4 mr-1" /> Conectado</span>
                                    ) : (
                                        <span className="flex items-center text-gray-400 text-sm font-medium"><XCircle className="h-4 w-4 mr-1" /> Desconectado</span>
                                    )}
                                </div>
                            ...
                            <div className="flex justify-end mt-auto">
                                {waStatus.connected ? (
                                    <button onClick={handleDisconnectWhatsApp} disabled={waLoading} className="px-4 py-2 border border-red-200 text-red-600 rounded-lg hover:bg-red-50 transition text-sm font-medium disabled:opacity-50">
                                        {waLoading ? 'Desconectando...' : 'Desconectar'}
                                    </button>
                                ) : (
                                    <button onClick={handleConnectWhatsApp} disabled={waLoading} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition flex items-center text-sm font-bold disabled:opacity-50">
                                        Conectar WhatsApp
                                    </button>
                                )}
                            </div>
                            ...
                            <div className="flex flex-col gap-3 mt-auto">
                                ...
                                <div className="flex justify-end">
                                    {tgStatus.connected ? (
                                        ...
                                        <button onClick={handleConnectTelegram} disabled={tgLoading || !tgToken} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition flex items-center text-sm font-bold disabled:opacity-50">
                    ...
                    <div className="bg-[#111827] rounded-xl border border-[#1f2937] overflow-hidden">
                        <div className="p-6 border-b border-[#1f2937]">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-white">Widget para Website</h2>
                                    <p className="text-sm text-gray-400">Incorpore um chat de IA em qualquer site com uma linha de código.</p>
                                </div>
                                <span className="ml-auto inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-green-900/30 text-green-400 border border-green-800/40">
                                    ✓ Ativo
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-5">
                                ...
                                <div>
                                    <label className="block text-sm text-gray-400 mb-2">Agente</label>
                                    <select value={selectedAgentId} onChange={e => setSelectedAgentId(e.target.value)}
                                        className="w-full bg-[#0b0e14] border border-[#374151] text-white text-sm rounded-lg px-3 py-2.5 focus:ring-purple-500 focus:border-purple-500 outline-none">
                                ...
                                <div>
                                    <label className="block text-sm text-gray-400 mb-2">Cor do Widget</label>
                                    <div className="flex items-center gap-3">
                                        <input type="color" value={widgetColor} onChange={e => setWidgetColor(e.target.value)}
                                            className="w-10 h-10 rounded-lg border border-[#374151] bg-transparent cursor-pointer" />
                                        <input type="text" value={widgetColor} onChange={e => setWidgetColor(e.target.value)}
                                            className="flex-1 bg-[#0b0e14] border border-[#374151] text-white text-sm rounded-lg px-3 py-2.5 outline-none font-mono" />
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {(['right', 'left'] as const).map(pos => (
                                            <button key={pos} onClick={() => setWidgetPosition(pos)}
                                                className={`py-2.5 rounded-lg text-sm font-semibold border transition-all ${widgetPosition === pos ? 'bg-purple-600 border-purple-600 text-white' : 'bg-[#0b0e14] border-[#374151] text-gray-400 hover:border-purple-500'}`}>
                            ...
                            {/* Right — Código */}
                            <div className="space-y-5">
                                <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Código de Incorporação</h3>
                                <p className="text-sm text-gray-400">Cole este código no <code className="text-purple-300">&lt;body&gt;</code> do seu site, antes de <code className="text-purple-300">&lt;/body&gt;</code>.</p>

                                {selectedAgentId ? (
                                    <div className="relative">
                                        <pre className="bg-[#0b0e14] border border-[#374151] rounded-xl p-4 text-xs text-green-300 overflow-x-auto whitespace-pre-wrap font-mono leading-relaxed">
                                            {widgetSnippet}
                                        </pre>
                                        <button onClick={copySnippet}
                                            className="absolute top-3 right-3 p-2 rounded-lg bg-[#1f2937] hover:bg-purple-900/40 text-gray-400 hover:text-purple-400 border border-[#374151] transition-all"
                                            title="Copiar código">
                                            {copied ? <Check className="h-4 w-4 text-green-400" /> : <Copy className="h-4 w-4" />}
                                        </button>
                                    </div>
                                ) : (
                                    <div className="bg-[#0b0e14] border border-[#374151] rounded-xl p-6 text-center text-gray-500 text-sm">
                                        Selecione um agente para gerar o código.
                                    </div>
                                )}

                                <div className="bg-blue-950/30 border border-blue-800/30 rounded-lg p-4">
                                    <p className="text-xs text-blue-300 font-semibold mb-1">💡 Como funciona</p>
                                    <p className="text-xs text-blue-200/70">O widget aparece como um botão flutuante no canto do seu site. Os visitantes podem conversar com o agente diretamente, sem sair da página.</p>
                                </div>
                            </div>
                        </div>
                    </div>
